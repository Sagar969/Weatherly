...
const TodaysHighlights = ({ wData }) => {
  return <>
    <div className="todays-highlights">
      <h5>Today's Highlights</h5>
      <div className="highlights-container">
        <WindStatus wData={wData} />
        <Humidity wData={wData} />
        <Visibility wData={wData} />
        <AirPressure wData={wData} />
      </div>
    </div>
    </>
}
...
const WindStatus = ({ wData }) => {
  const windSpeed = wData.hourly.windspeed_10m[0];
  const dirDegree = wData.hourly.winddirection_10m[0];
  const direction = checkDirection(dirDegree);
  return (
    <div>
      <p>Wind Status</p>
      <span><h2>{windSpeed}</h2><h4>km/h</h4></span>
      <span className="wind-direction"><img src={directionIcon} alt="Wind direction" style={{transform: `rotate(${-46 + dirDegree}deg)`}} /><p>{direction}</p></span>
    </div>
  )
}
const Humidity = ({ wData }) => {
  const [humidity, setHumidity] = useState(0);
  const humi = wData.hourly.relativehumidity_2m[0];
  
  useEffect(() => {
    setHumidity(humi);
    document.querySelector('input[name="humidity"]').style.backgroundSize = `${humidity}% 100%`;
  }, [humidity])

  return (
    <div>
      <p>Humidity</p>
      <span><h2>{humidity}</h2><h4>%</h4></span>
...
const Visibility = ({ wData }) => {
  const vis = Math.round((wData.hourly.visibility[0]) / 100) / 10;
  return (
    <div>
      <p>Visibility</p>
      <span><h2>{vis}</h2><h4>km</h4></span>
    </div>
  )
}
const AirPressure = ({ wData }) => {
  const pressure = wData.hourly.pressure_msl[0];
  return (
    <div>
      <p>Air Pressure</p>
      <span><h2>{pressure}</h2><h4>mb</h4></span>
    </div>
  )
}
