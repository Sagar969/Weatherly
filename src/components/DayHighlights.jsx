      ...
      <div className="hl-container">
       ...
       <div className="weather-details">
          ...
          <div className="max-temp">
            <h5>Max temp</h5>
            <h3>{maxTemp}<span>{tUnit}</span></h3>
          </div>
          <div className="min-temp">
            <h5>Min temp</h5>
            <h3>{minTemp}<span>{tUnit}</span></h3>
          </div>
       </div>
...
const Humidity = ({ wData, dayNum }) => {
  ...
  useEffect(() => {
    setHumidity(humi);
    document.querySelector('#hl-humidity').style.backgroundSize = `${humidity}% 100%`;
  }, [humidity])
  ...
  return (
    <div>
      ...
      <div className="range-bar-box">
        ...
        <input type="range" name="humidity" min="0" max="100" id="hl-humidity"/>
        <p>%</p>
      </div>
    </div>
  )
...
const Visibility = ({ wData, dayNum }) => {
  const vis = Math.round((wData.hourly.visibility[0 + curHour + (dayNum * 24)]) / 100) / 10;
  return (
    <div>
      <p>Visibility</p>
